...
contract NftCollection is ERC721, ERC721Enumerable, AccessControl, INftCollection {
    ...
    constructor(
        ...
        // Setting router as admin and minter
    ...
    function diamondMint(address owner, string memory _tokenURI) public returns (uint256) {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'ERC721PresetMinterPauserAutoId: must have admin role to mint');
        require(hasRole(MINTER_ROLE, owner), 'ERC721PresetMinterPauserAutoId: must have minter role to mint');
        require(!_tokenURIExists[_tokenURI], 'The token URI should be unique');
        tokenURIs.push(_tokenURI);
        uint256 _id = tokenURIs.length; // based on 1
        _tokenIdToTokenURI[_id] = _tokenURI;
